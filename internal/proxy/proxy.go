...
func Start(ctx context.Context, runner executor.Runner, logger audit.Logger, cfg model.Config, runDir string, req model.ProxyRequest) (model.ProcessState, error) {
	if req.LocalHost == "" {
	...
	if req.ID == "" {
		req.ID = fmt.Sprintf("proxy_%d", time.Now().UnixNano())
	}

	args := sshargs.BuildProxyArgs(cfg, req.ResolvedHost, req)
...
func List(runDir string) ([]model.ProcessState, error) {
	return loadStates(runDir)
}

func Stop(runDir string, id string) error {
	return stopState(runDir, id)
}

func loadStates(runDir string) ([]model.ProcessState, error) {
...
func stopState(runDir string, id string) error {
